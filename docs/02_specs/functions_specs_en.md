...
### Purpose
- Adds metadata columns for tracking, audit, and lineage purposes within the ETL pipeline.
- Added columns may include:
    - Processing timestamp (`process_date`)
    - Source file path (`source_file`)

### Input
- Processing dataset.
- Columns to be added and their schema definition.

### Output
- Success: Dataset contains the newly added columns.
...
| Scenario | Expected Result |
|----------|----------------|
| Rule contains process_date | process_date column is added to all records |
| Rule contains source_file | source_file column is added to all records |
| Rule contains both columns | Both process_date and source_file are added |
| Rule contains unsupported column | Message `Unknown column name` |
| Input data contains N records | Output still contains N records |
| No data is modified except metadata columns | Existing column values remain unchanged |
...
### Failure Example
- Add a column that is not defined in the configuration.
